# Parrot-MCP Makefile
# This Makefile provides a set of commands to manage the Parrot-MCP project.
...
venv:
	...
	@echo 'run `source .venv/bin/activate` to start develop with Nav-MCP.'
...
# Format code
format:
	uv run black Nav-MCP

# Lint code
lint:
	uv run pylint --rcfile .pylint Nav-MCP/*.py
	uv run black --check Nav-MCP

# Run tests with coverage
test:
	uv run coverage run -m Nav-MCP.tests
	uv run coverage report
	uv run mypy Nav-MCP/*.py
...
bump-patch:
	@python -c "import re; \
	content = open('Nav-MCP/version.py').read(); \
	version = re.search(r'__version__ = \"(.+)\"', content).group(1); \
	parts = version.split('.'); \
	parts[2] = str(int(parts[2]) + 1); \
	new_version = '.'.join(parts); \
	new_content = re.sub(r'__version__ = \".+\"', f'__version__ = \"{new_version}\"', content); \
	open('Nav-MCP/version.py', 'w').write(new_content); \
